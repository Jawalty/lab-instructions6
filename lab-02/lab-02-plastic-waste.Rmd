...
**Your answer:** The source pane is where we load our rmd files and edit them accordingly. The console pane is where we load our libraries or separate commands to then use in our source pane. The environment pane is where we can load some data sets, look at our work history, as well as push and commit our work changes back to our forked repositories. Finally the files pane is where all of our files and packages are stored, as well as our viewer pane which enables us to see our final product.
...
**Your answer:** The dataset has __240___ observations.
...
**Your answer:**
Europe has the highest while South America has the lowest. North America is showing the most varietal data.
...
--- I think 0.3 works the best to see the outlines of each different continent, but 0.5 works well to see the colors brighter, which would be better for observing specific countries one at a time.
...
**Your answer:** Color and fill go inside of aes() because they both vary based on the data, but mapping alpha inside geom_density() doesn't have it be varied by the data, but instead apply to the visualization of it.
