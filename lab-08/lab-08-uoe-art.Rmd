...
- ✅ Watched lectures on functions, iteration, and web scraping
- ✅ **Installed SelectorGadget browser extension** (see instructions below)
- ✅ Forked the lab-instructions repository
- ✅ Understand HTML basics and CSS selectors
...
✅ **Verified!** We can proceed with scraping.
...
🧶 ✅ ⬆️ **Remember to commit and push after completing each script!**
...
```{r load-scraped-data, eval = TRUE}
#remember to switch eval = FALSE to TRUE when you have your data
uoe_art <- read_csv("/home/jovyan/lab-instructions8/data/uoe_art.csv")
```
...
```{r separate-title-date, eval = TRUE}
...
**Fill in the blanks:**
```{r extract-year, eval = TRUE}
# Remember to change eval=FALSE to eval=TRUE!
uoe_art <- uoe_art %>%
  mutate(
    year = str_extract(title, "\\d{4}"),     # Extract 4-digit number
...
- Number of pieces with missing artist: 108
- Number of pieces with missing year: 782
...
```{r year-histogram, eval = TRUE}
# Remember to change eval=FALSE to eval=TRUE!
ggplot(data = uoe_art, aes(x = year)) +
  geom_histogram(binwidth = 10) +
  labs(
    title = "Distribution of uoe art collection by year",
    x = "year",
    y = "count"
  )
```
...
**What looks out of the ordinary?** There's an outlier in the 1700's.



🧶 ✅ ⬆️ **Knit, commit with message "Completed Exercises 10-11", and push your changes.**
...
**Visit the link for this piece and find the correct year:** It returned zero, I'm confused on how to find the outlier piece.
...
**Correct the error:**
```{r fix-year, eval = TRUE}
# Remember to change eval=FALSE to eval=TRUE!
uoe_art <- uoe_art %>%
  mutate(year = if_else(
    title == "Mary Ann Park Sampler",          # Condition: if title equals the problematic one
    1819,                    # Then: set year to correct value
    year                     # Else: keep original year
  ))
```

**Recreate the histogram to verify the fix:**
```{r verify-fix, eval = TRUE}
# Remember to change eval=FALSE to eval=TRUE!
ggplot(data = uoe_art, aes(x = year)) +
  geom_histogram(binwidth = 10) +
  labs(
...
```{r most-common-artist, eval = TRUE}
# Remember to change eval=FALSE to eval=TRUE!
uoe_art %>%
...
**Most common artist:** Is Unknown, but the highest known artist is Emma Gillies.

**Number of pieces:** Unknown has 6 and Emma has 3.

**Do you know this artist?**  I personally do not, but I am not well diversed in the artist realm.




**Why might the university have so many of their pieces?** They could be historical and represent parts of history that are difficultly portrayed through words.
...
**Number of pieces with "child" in the title:** Zero for both.

🧶 ✅ ⬆️ **Knit, commit with message "Completed Exercise 14", and push your changes.**
...
- **HTTP errors (403, 404, etc.)** → The website may be down or blocking requests
- **Different number of elements** → Website structure may have changed since lab was written
- **Function returns wrong number of rows** → Check that CSS selectors are correct

**Data cleaning errors:**

- **`separate()` warnings** → Expected! Some titles don't have parentheses
- **Year extraction fails** → Check that pattern `\\d{4}` is correct
- **NA values everywhere** → Make sure you saved the result back to `uoe_art` with `<-`

**String manipulation errors:**

- **Error with `\\(`** → Need double backslash to escape special characters
- **`str_extract()` returns NA** → Pattern might not match; check your regex
- **Case sensitivity issues** → Use `(?i)` for case-insensitive matching

**Visualization errors:**

- **Histogram looks weird** → Adjust binwidth or check for outliers
- **Too many NA values** → Use `na.rm = TRUE` or filter them out first

**General issues:**

- **Can't find CSV file** → Make sure you ran all three scraping scripts and saved the data
- **Code chunk not running** → Make sure you changed `eval=FALSE` to `eval=TRUE`
- **Warnings about coercion** → Expected when converting text to numbers (some won't convert)

---

**Final check:** 
- ✅ All three R scripts completed and committed
- ✅ CSV file in data folder
- ✅ All exercises in this .Rmd completed
