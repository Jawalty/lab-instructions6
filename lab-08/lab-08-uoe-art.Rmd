...
uoe_art <- uoe_art %>%
  mutate(
    year = str_extract(date, "\\d{4}"),     # Extract 4-digit number
...
# Remember to change eval=FALSE to eval=TRUE!
uoe_art %>%
  filter(year < 1500 | year > 2030)  # Look for unreasonable years
```
...
uoe_art <- uoe_art %>%
  mutate(year = if_else(
    title == "___",          # Condition: if title equals the problematic one
